...
const ALLORA_CONSUMER_CHAIN_ID = 245022926;
...
const constructMessageLocally = async (
  ...
  const networkInferenceTypehash = keccak(
    toBytes(
...
const signMessageLocally = async (
  ...
  const localMessage = await constructMessageLocally(numericData, {
    chainId,
    alloraConsumerAddress,
  });
...
const run = async () => {
  ...
  const signerPrivateKey = getEnvVariable("privateKey");
  ...
  const alloraConsumer = new AlloraConsumer__factory()
    ...
    .connect(signerWallet) as AlloraConsumer;
  ...
  const messageFromRemoteContract =
    await alloraConsumer.getNetworkInferenceMessage(networkInferenceData);
